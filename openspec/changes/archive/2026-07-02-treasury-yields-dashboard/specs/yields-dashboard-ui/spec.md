...
Note: the visual layout, IA, and state depictions are fixed by the accepted
`design/layout.svg` and `design/ux-brief.md` (from `dev-design`). These
requirements state the behaviour that implementation SHALL satisfy; they do not
redefine the visual design.
